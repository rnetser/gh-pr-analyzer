...
"""
    ...
    for analysis in analyses:
        ...
        if analysis.review_labels:
            ...
            if lgtm_users:
                review_items.append('<div style="margin-bottom:4px"><strong>LGTM:</strong></div>')
                for user in lgtm_users:
                    review_items.append(f'<div><span class="status-badge status-passing">👍 {escape(user)}</span></div>')
            if changes_users:
                review_items.append('<div style="margin-bottom:4px"><strong>Changes requested:</strong></div>')
